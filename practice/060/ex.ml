...
module Make(Tested: Testable) : sig val run : unit -> unit end = struct
  ...
  let test_hbal_tree_nodes () =
    let test_height_1 () =
      ...
      check (list (tree_pp Format.pp_print_char)) "Height 1" expected_result trees
    in
    ...
    let test_height_2 () =
      ...
      check (list (tree_pp Format.pp_print_char)) "Height 2" expected_result trees
    in

    let test_height_0 () =
      let trees = hbal_tree_nodes 0 in
      let expected_result = [Empty] in
      check (list (tree_pp Format.pp_print_char)) "Height 0" expected_result trees
    in
