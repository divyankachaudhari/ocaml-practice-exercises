...
*)
  ...
  let test_example_test () =
    ...
    check (of_pp (trees_pp (fun out -> Format.fprintf out "%c"))) "Example Test" expected_result result

  let test_empty_tree () =
    let result = cbal_tree 0 in
    let expected_result = [Empty] in
    check (of_pp (trees_pp (fun out -> Format.fprintf out "%c"))) "Empty Tree Test" expected_result result

  let test_single_node () =
    let result = cbal_tree 1 in
    let expected_result = [Node ('x', Empty, Empty)] in
    check (of_pp (trees_pp (fun out -> Format.fprintf out "%c"))) "Single Node Test" expected_result result
  ...
  let test_multi_nodes () =
    ...
    check (of_pp (trees_pp (fun out -> Format.fprintf out "%c"))) "Multi Nodes Test" expected_result result
