...
(*
  let tree_equal a b = 
    ...
    | _ -> false
*)
  let test_layout_binary_tree_2_works_correctly () =
    ...
    check (of_pp (tree_pp (fun fmt (v, x, y) -> Format.fprintf fmt "('%c', %d, %d)" v x y)))
      "test_layout_binary_tree_2_works_correctly" expected_layout (layout_binary_tree_2 example_layout_tree)

  let run () =
    let open Alcotest in
    run "layout_binary_tree_2" [
      "layout_binary_tree_2", [
        test_case "test_layout_binary_tree_2_works_correctly" `Quick test_layout_binary_tree_2_works_correctly;
      ]
    ]
end
